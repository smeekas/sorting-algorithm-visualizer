...
def paint(numbers,colortype):       #this function will draw the array
    ...
    if TYPE==0:
        ...
        for i in range(len(numbers)):
            ...
            canva.create_rectangle(starting_point,0,starting_point+rec_width,CANVAS_Y,outline="",fill="#%02x%02x%02x"%(int(red),int(green),int(blue)))
            starting_point+=rec_width
    frame2.update()
...
def sort_list():            #this function will sort the list with seleted sorting algorithm
    ...
    algochooser(numbers,paint,label_access,label_comparison,algo_var.get(),TYPE)
...
frame_radio=Frame(frame1,bg="#DEE6DB",width=230,height=80,relief="flat",bd=4)
frame_radio.place(relx=0,rely=0.43)
frame_radio.grid_propagate(0)

bar_drawing=Radiobutton(frame_radio,text="Bar drawing",bg="#DEE6DB",variable=graph_type,value=0,font=("Helvetica",10,"bold"),command=draw_type)
color_drawing=Radiobutton(frame_radio,text="color drawing",bg="#DEE6DB",variabl=graph_type,value=1,font=("Helvetica",10,"bold"),command=draw_type)
...
scale_size=Scale(frame1,label="Size :",orient=HORIZONTAL,from_=20,to=200,length=230,bg="#d6e0d2",troughcolor="#024e76",variable=size_var,command=change_size,relief="solid")
...
rec_width=CANVAS_X//size_var.get()
for num in numbers:
    canva.create_rectangle(starting_point,CANVAS_Y-num,starting_point+rec_width,CANVAS_Y,fill="#aaaaaa")
    starting_point+=rec_width
