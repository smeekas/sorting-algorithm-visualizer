...
def algochooser(numbers, paint, label_access, label_comparison, something, TYPE_OF_DRAW):
    ...
    if something == "bubble sort":
        ...
        label_access.configure(text="array access:0")
        label_comparison.configure(text="comparison:0")
        mergesort(numbers, 0, len(numbers), paint,
                  label_access, label_comparison)
        if TYPE == 0:
            paint(numbers, ["green"]*len(numbers))
        ac, cmp = 0, 0
        ...
        label_comparison.configure(text="comparison:0")
        quicksort(numbers, 0, len(numbers)-1, paint,
                  label_access, label_comparison)
        if TYPE == 0:
            paint(numbers, ["green"] * len(numbers))
        ac, cmp = 0, 0
...
def bubblesort(number, paint, label_access, label_comparison):
    ...
    for i in range(len(number)-1):
        ...
        for j in range(len(number)-1-i):
            ...
            # --------------------------------------------
            if TYPE == 0:
                colors = ["#cc0000"if x == number[j] or x ==
                          number[j+1] else"#aaaaaa" for x in number]
            else:
...
def selectionsort(number, paint, label_access, label_comparison):
    ...
    for i in range(len(number)-1):
        for j in range(i+1, len(number)):
            ...
            cmp += 1
            if TYPE == 0:
                colors = ["#cc0000"if x == number[j] else "#aaaaaa" if x !=
                          number[i] else "green" for x in number]
            else:
                colors = [((int)(x * 360) / 950) for x in number]
            paint(number, colors)
            label_access.configure(text="array access:"+str(ac))
...
def mergesort(number, left, right, paint, label_access, label_comparison):
    if left < right:
        ...
        mergesort(number, middle+1, right, paint,
                  label_access, label_comparison)
        merge(number, left, middle, right, paint,
              label_access, label_comparison)
...
def quicksort(number, left, right, paint, label_access, label_comparison):
    ...
    if(left < right):
        mid = partition(number, left, right, paint,
                        label_access, label_comparison)
...
def shellsort(number, paint, label_access, label_comparison):
    ...
    while gap > 0:  # loop until gap #cc0000uced to 1. -if array size is 20 then gap=10,5,2,1
        # loop srart from gap bcoz we are using j=x_sort-gap
        for x_sort in range(gap, length):
            ...
            if TYPE == 0:
                colors = ["#cc0000" if xy == j + gap or xy ==
                          j else "#aaaaaa" for xy in range(len(number))]
            else:
                colors = [((int)(x * 360) / 950) for x in number]
            paint(number, colors)
            # ---------------------------------------------------------
            # if you swap element then go back by gap(j-=gap) and swap and if you haven't swapped then break the loop so you can go forward
            while j >= 0:
    ...
    while is_sort == False:
        ...
        for x in range(0, length-1, 2):
            if number[x] > number[x+1]:
                ...
                # --------------------------------------------------------------------------------
                if TYPE == 0:
                    colors = ["#cc0000" if v == x or v == x +
                              1 else "#aaaaaa" for v in range(len(number))]
                else:
                    colors = [((int)(x * 360) / 950) for x in number]
                paint(number, colors)
                colors = []
